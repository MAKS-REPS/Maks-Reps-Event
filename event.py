...
class Event(commands.Cog):
    ...
    async def odbierz(self, interaction: discord.Interaction):
        ...
        embed_main = discord.Embed(title="Zgłoś wykonanie zadania", color=KOLOR_BIALY)
        embed_main.description = "Wybierz zadanie z listy poniżej, aby otworzyć ticket."
        embed_main.set_footer(text=f"{NAZWA_EVENTU} | Dziś o {datetime.now().strftime('%H:%M')}")

        await interaction.response.send_message(embed=embed_main, view=TicketView(), ephemeral=True)
    ...
    async def ranking(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title="Ranking TOP 10 eventu", description=desc if desc else "Brak danych.", color=KOLOR_BIALY)
        embed.set_footer(text=f"{NAZWA_EVENTU} | Dziś o {datetime.now().strftime('%H:%M')}")
        await interaction.response.send_message(embed=embed)
